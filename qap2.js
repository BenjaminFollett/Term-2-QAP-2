...
function parseDateString(value) {
  if (typeof value !== 'string') {
    throw new Error('This is totally wrong.');
  }

  const regex = /^\d{4}-\d{2}-\d{2}$/;
  if (!regex.test(value)) {
    throw new Error ('Invalid date string format. please try again.');
  }
...
try {
  console.log(parseDateString('2024-03-16'));
} catch (error) {
...
try {
  const parsedDate = parseDateString('2024-03-16');
  console.log(toDateString(parsedDate));
} catch (error) {
  console.error(error.message);
}
...
function normalizeCoord(value) {
  ...
  let coords = (match[1] || match[2]).split(',');
...
const value = "42.9755,-77.4369";
const formattedCoordinates = normalizeCoord(value);
...
function formatCoords(...values) {
  const validCoords = values.map(normalizeCoord).filter(values => values !== null);
  return `(${validCoords.join(', ')})`;
}

const result = formatCoords("42.9755,-77.4369", "[-62.1234, 42.9755]","300,-9000");
console.log(result);
...
console.log(mimeFromFilename('example.html'));
console.log(mimeFromFilename('image.jpeg'));
...
function generateLicenseLink(licenseCode, targetBlank = false) {
  ...
  let LicenseWords;
  switch (FixedLicenseCode) {
    case 'by': LicenseWords = 'Creative Commons Attribution License';
    break;
    case 'by-nc': LicenseWords = 'Creative Commons Attribution-NonCommercial License';
    break;
    case 'by-nd': LicenseWords = 'Creative Commons Attribution-NoDerivs License';
    break;
    case 'by-sa': LicenseWords = 'Creative Commons Attribution-NonCommercial-ShareAlike License';
    break;
    case 'by-nc-nd': LicenseWords = 'Creative Commons Attribution-NonCommercial-NoDerivs License';
    break;
    case 'by-nc-sa': LicenseWords = 'CC-BY-NC-SA: Creative Commons Attribution-NonCommercial-ShareAlike License';
    break;
    default: LicenseWords = 'Creative Commons License';
}

const LicenseUrl = `${BaseUrl}${FixedLicenseCode}/4.0/`;
const LinkAttributes = targetBlank ? 'target="_blank"': '';
const LinkHtml = `<a href="${LicenseUrl}"${LinkAttributes}>${LicenseWords}</a>`;
...
function pureBool(value) {
  ...
  const TrueValues = ['Yes', 'yes', 'YES', 'Y', 'Oui', 'oui', 'OUI', 'O', 't', 'TRUE', 'true', 'True', 'vrai', 'V', 'VRAI', '1', '2'];
  ...
  if (TrueValues.includes(ValueUpper) || (!isNaN(value) || parseInt(value) > 0)) {
    return true;
  } else if (FalseValues.includes(ValueUpper) || (!isNaN(value) || parseInt(value) <= 0)) {
    return false;
  }
...
console.log(pureBool(-1));
/*console.log(pureBool('unexpected value'));*/
...
function every(...lists) {
  try {
    return lists.every(list => pureBool(list));
  } catch (error) {
    return false;
  }
}
...
function any(...lists) {
  try {
    return lists.some(list => pureBool(list));
  } catch (error) {
    return false;
  }
}
...
function buildUrl(query, order, count, license) {
  ...
  if (!ValidLicenses.includes(license)) {
    throw new Error('Invalid license value. please re-enter something with' + ValidLicenses.join(', ') + '.');
  }
  const CodedQuery = encodeURIComponent(query);
  const url = `https://api.inaturalist.org/v2/observations?query=${CodedQuery}&count=${count}&order=${order}&license=${license}`;
  return url;
}
